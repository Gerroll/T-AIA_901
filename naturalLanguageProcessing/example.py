...
TRAIN_DATA = [
    ...
    (
        ...
        {
            "heads": [0, 0, 3, 3, 5, 5, 7, 3, 9, 7],
            "deps": ["-", "-", "-", "MOVE", "-", "MOVE", "-", "START", "-", "END"]
        }
    ),
    (
        "Je voudrais aller demain de Nîmes à Paris pour 8h",
        {
            "heads": [1, 2, 2, 2, 5, 2, 7, 5, 9, 3],
            "deps": ["-", "-", "MOVE", "TIME", "-", "START", "-", "END", "-", "TIME"]
        }
    ),
    (
...
def test_model(nlp):
    ...
    for doc in docs:
        ...
        print(doc.text)
        print([(t.text, t.dep_, t.head.text) for t in doc if t.dep_ == "END" or t.dep_ == "MOVE" or t.dep_ == "START"])
